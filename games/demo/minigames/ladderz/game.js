...
preloadGame.prototype = {
    preload: function(){
        ...
        game.load.image("diamond", "coin.png");
...
playGame.prototype = {
    ...
    createOverlay: function(){
        ...
        this.scoreText = game.add.text(10, game.height - 10, "Score: 0", { font: '50px' });
    ...
    createMenu: function(){
        ...
        var tapText = game.add.text(game.width / 2, tap.y - 120, "Tap to jump", { font: '45px'});
        tapText.anchor.set(0.5);
        this.menuGroup.add(tapText);
        var titleText = game.add.text(game.width / 2, tap.y - 200, "LADDERZ", { font: '90px'});
    ...
    addHero: function(){
        ...
        this.hero.body.onWorldBounds.add(function(sprite, up, down, left, right){
            ...
            if(down){
                if (!this.end) {
                  this.end = true
                  var score = this.reachedFloor + this.collectedDiamonds * 3;
                  setTimeout(() => {
    ...
    checkLadderCollision: function(){
        if(!this.isClimbing){
            ...
            if(this.hero.y < this.ladderToClimb.y - 40){
                ...
                this.isClimbing = false;
                this.reachedFloor ++;
                this.scoreText.text = 'Score: ' + (this.collectedDiamonds * 3 + this.reachedFloor).toString();
            }
        }
    },
...
var audio = new Audio('music.mp3?v=2');
audio.loop = true
let successPlaying = false
function playAudio() {
  if (audio.played > 0 || successPlaying) return
  audio.play().then(() => successPlaying = true).catch(e => audio.play())
}

window.addEventListener('load', (event) => {
  const bodyTag = document.getElementsByTagName('body')[0]
  bodyTag.addEventListener("touchstart", playAudio, false);
  bodyTag.addEventListener("touchend", playAudio, false);
});
